...
export default function ProductsListComponent({ 
  ...
  // Usar el contexto para actualizar el footer (solo si es standalone)
  const { 
    setTotalProducts, 
    setFilteredProducts, 
    setHasActiveFilters, 
  ...
  }, [isStandalone, setIsLoading, setTotalProducts, setFilteredProducts, setHasActiveFilters]);
  ...
  const handleApplyFilters = (filters: FilterState) => {
    ...
    // Simular filtrado
    setTimeout(() => {
      const hasFilters = filters.categories.length > 0 && !filters.categories.includes('all') ||
                        filters.sortBy !== 'name' ||
                        filters.status !== 'all';
  ...
  return (
    <div className={`relative ${className}`}>
      {/* Lista de productos con SCROLL PROPIO */}
      <div 
        className={`${isStandalone ? '' : `max-h-[${maxHeight}] overflow-y-auto`}`}
