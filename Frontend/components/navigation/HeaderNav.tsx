    ...
    <div className={`${isFixed ? 'fixed top-[80px]' : ''} flex justify-between items-center p-4 bg-white border-b border-gray-200 left-0 right-0 z-40 safe-area-top pt-[calc(1rem+env(safe-area-inset-top))] 
      ...
      <div className="flex items-center gap-2 justify-between w-full pt-[10px] px-4 touch-target">
        ...
        >
          ...
          <span className="text-[18px] font-semibold transition-interactive">{title}</span>
        </button>
        <div className="flex items-center gap-2">
          {isCartPage && hasItems && (
            <button
              className="text-red-600 font-semibold text-[14px] px-2 py-1 rounded hover:bg-red-50 
                       transition-interactive gpu-accelerated active:scale-95"
              onClick={clearCart}
              aria-label="Leeren"
              tabIndex={0}
            >
          ...
          {isPromotionPage && (
            <div className="flex items-center gap-0.5">
              <Zap className="w-4 h-4 text-black transition-interactive" />
              <span className="text-[14px] font-semibold transition-interactive">
          ...
          {!isCartPage && !showAddButton && !isPromotionPage && (
            <button
              ...
              tabIndex={0}
            >
              <X className="w-5 h-5 text-gray-600 transition-interactive" />
            </button>
          )}
          {showAddButton && (
            <button
