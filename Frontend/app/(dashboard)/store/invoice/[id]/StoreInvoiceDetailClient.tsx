...
interface StoreInvoiceDetailClientProps {
  invoiceId: string;
}

export default function StoreInvoiceDetailClient({
  invoiceId,
}: StoreInvoiceDetailClientProps) {
    ...
    <div className="invoice-print-container w-full h-full">
      {isMobile && (
        <div className="w-full">
          <InvoiceTemplate invoice={invoice} showActions={false} isMobile={true} />
        </div>
      )}
      {!isMobile && (
        <div className="p-6 max-w-4xl mx-auto">
          <InvoiceTemplate invoice={invoice} showActions={true} isMobile={false} />
        </div>
      )}
    </div>
  );
