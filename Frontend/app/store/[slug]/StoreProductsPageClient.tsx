...
export default function StoreProductsPageClient() {
  ...
  // Chips en layout: solo id/label/count (sin iconos React — no son serializables)
  const { categoryFilters, categoryFiltersSignature } = useMemo(() => {
    if (allProducts.length === 0) {
      return { categoryFilters: [], categoryFiltersSignature: '' }
    }
    ...
    const filters = [
      { id: 'all', label: 'Alle', count: allProducts.length },
    ...
    const signature = filters.map((f) => `${f.id}:${f.count}:${f.label}`).join('|')
    return { categoryFilters: filters, categoryFiltersSignature: signature }
  }, [allProducts])

  const prevFiltersRef = useRef('')
  useEffect(() => {
    if (prevFiltersRef.current === categoryFiltersSignature) return
    prevFiltersRef.current = categoryFiltersSignature
    storeContext.setCategoryFilters(categoryFilters)
  }, [categoryFiltersSignature, categoryFilters, storeContext.setCategoryFilters])
